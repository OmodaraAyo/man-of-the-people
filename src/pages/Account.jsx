...
const Account = () => {
  ...
  return (
    <div className="pt-42">
      <div className="w-full bg-gray-200 flex items-center p-1.5 rounded container mx-auto border-none gap-2 text-black max-w-[21.3rem] sm:max-w-[32rem] z-30">
              ...
              {showWarning && (
                  <div className="w-full h-full bg-[#ffefbc] flex justify-around py-5 px-4 sm:px-9 container mx-auto sm:max-w-[32rem] mt-0.5 rounded">
              ...
              <div className="flex flex-col gap-4 w-full items-center bg-gray-200 mt-4 py-5 px-7 sm:px-11 insert-x-0 mx-auto max-w-screen-md flex-grow">
                  ...
                  <div key={index} className="w-full h-full max-w-[33rem] rounded pt-2"style={{background: "linear-gradient(135deg, #8B0000 55%, #00008B 45%)"}}>
                      <div className="flex flex-col gap-3 bg-white w-full h-full shadow-lg rounded-b pb-4">
                          <h1 className="flex justify-between px-4 py-2 font-bold text-[1.169rem] font-sans border-b border-gray-300">
                              <p>{account.name}</p>
                              <div className="cursor-pointer">{activeAccount ? <MdKeyboardArrowUp size={24} onClick={() => setActiveAccount(false)} /> :  <MdKeyboardArrowDown size={24} onClick={() => setActiveAccount(true)}/>}</div>
                          </h1>
                          ...
                          <div className="flex flex-col px-4 font-semibold text-[0.999rem]">
                              ...
                              {activeAccount && account.text ? <p className="italic text-[0.573rem] font-semibold"><span className="text-blue-950 font-extrabold not-italic text-[0.955rem]">FDIC</span> {account.text}</p>: '' }
                          </div>
                          {activeAccount && (
                            <div className="flex justify-between px-4 font-medium">
                            <h1 className="max-w-[9rem]">{account.type}</h1>
                            <Link to={"/home/account/history"} className="font-bold text-[1.22rem] flex items-center cursor-pointer" ><p>{account.balance}</p> <MdKeyboardArrowRight size={24} /></Link>
                        </div>
                          )}
                      </div>
                  </div>
